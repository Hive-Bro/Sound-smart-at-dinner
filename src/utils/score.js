...
export function scoreStories(stories) {
  return stories
    .map((story) => {
      ...
      for (const word of SENTIMENT_WORDS.positive) if (text.includes(word)) sentimentScore++;
      for (const word of SENTIMENT_WORDS.negative) if (text.includes(word)) sentimentScore--;
      ...
      return { ...story, categories: categories.length > 0 ? categories : ['general'], sentiment, relevance };
    })
